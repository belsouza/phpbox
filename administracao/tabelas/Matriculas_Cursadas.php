...
error_reporting(0);
   ...
   class Matriculas_Cursadas{
      ...
      private function insert( $nome, $cpf, $datanascimento ){         
         ...
         $verificar = "SELECT Nome, CPF FROM matriculas_cursadas WHERE CPF = {$cpf} AND Matricula = {$id}";
         ...
         if($cont == 0){            
            ...
            $sql = "INSERT INTO matriculas_cursadas ( Matricula, Nome, CPF, DataNascimento) VALUES ( '{$id}', '{$nome}', '{$cpf}', '{$datanascimento}')";            
      ...
      private function update( $matricula, $nome, $cpf, $datanascimento ){
         ...
         $sql = "UPDATE matriculas_cursadas SET ";
      ...
      private function delete( $matricula ){
         ...
         $sql = "DELETE * FROM matriculas_cursadas WHERE Matricula={$matricula}";
         if($this->conn->query($sql) === TRUE){
      ...
      private function select( $args , $matricula ){
         ...
         if($args == ""){
            ...
            $sql = "SELECT * FROM matriculas_cursadas";
         ...
         else
         {
            ...
            $sql = "SELECT {$args} FROM matriculas_cursadas WHERE Matricula = {$matricula}";
            $result = $this->conn->query( $sql );
      ...
      private function search ( $palavra ){
         ...
         $sql = "SELECT * FROM matriculas_cursadas WHERE ";
      ...
      public function exibir_matriculas_cursadas(){         
         return $this->select("", "");         
      }

      public function inserir_matriculas_cursada( $nome, $cpf, $datanascimento ){
         return $this->insert( $nome, $cpf, $datanascimento );             
      }

      public function atualizar_matriculas_cursadas( $matricula, $nome, $cpf, $datanascimento ){
         return $this->update($matricula, $nome, $cpf, $datanascimento);         
      }

      public function excluir_matriculas_cursadas( $matricula, $nome, $cpf, $datanascimento ){
