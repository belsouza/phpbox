<?php
    ...
    require_once "tabelas/TMatriculas_Cursadas.php";
    ...
    $matriculas_cursadas = new matriculas_cursadas();
    $validar = new Validar();
    ...
    if($_SERVER["REQUEST_METHOD"] === 'POST'){
        ...
        if(isset($_POST["inserir"])){
            ...
            $mensagem = $matriculas_cursadas->inserir_aluno( $nome, $cpf, $datanascimento ); 
        }        
    ...
    if(isset($_GET["editar"]) or isset($_GET["excluir"])  ){
        ...
        if(isset($_GET["editar"])){
            ...
            $consulta_nome = $matriculas_cursadas->exibir_nome( $consulta_matricula );
            $consulta_cpf = $matriculas_cursadas->exibir_cpf( $consulta_matricula );
            $consulta_datanascimento = $matriculas_cursadas->exibir_datanascimento(  $consulta_matricula );
            $consultatabela = "";
            $confirma = "Atualizar";
            $editar = TRUE;
        }
        ...
        if(isset($_POST["operacao"])){
            ...
            if($_POST["operacao"] == "Atualizar"){
                ...
                $mensagem = $matriculas_cursadas->atualizar_aluno( $matricula, $nome, $cpf, $datanascimento );
            ...
            if($_POST["operacao"] == "Excluir"){
                ...
                $mensagem = $matriculas_cursadas->excluir_aluno( $matricula );
                $consulta_matricula = $consulta_nome = $consulta_cpf = $consulta_datanascimento = $consultatabela = "";                 
                $editar = FALSE; 
            }
        }
    ...
    {
        $consultatabela = $matriculas_cursadas->procurar($_GET["procurando"]);
        $tabela = new Tabela( $consultatabela );
        $consulta = $tabela->exibir();
    }
    else{
        $consultatabela = $matriculas_cursadas->exibir_matriculas_cursadas();
        $tabela = new Tabela( $consultatabela );
        $consulta = $tabela->exibir();
    }
?>